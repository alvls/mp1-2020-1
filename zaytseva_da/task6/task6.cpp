...
enum ConsoleColor //чтоб можно было поиграться с цветами
{
	Black = 0,
	Blue = 1,
	Green = 2,
...
public:
	...
	void DrawSnake(int headcolor, int bodycolor)
	{
		cls(hConsole);
		setposition(snake[0].x, snake[0].y);
		SetConsoleTextAttribute(hConsole, (WORD)((Black) | headcolor));
		cout << "O";
		for (int i = 1; i < tail; i++)
		{
			setposition(snake[i].x, snake[i].y);
			SetConsoleTextAttribute(hConsole, (WORD)((Black) | bodycolor));
			cout << "o";
		}
	}
	void crawl()
...
public:
	...
	{
		SetConsoleTextAttribute(hConsole, (WORD)((Black) | color));
		for (int j = 0; j < 2; j++)
		{
			for (int i = 0; i < n; i++)
			{
...
public:
	void DrawEAT(int foodcolor)
	{
		...
		{
			SetConsoleTextAttribute(hConsole, (WORD)((Black) | foodcolor));
...
int main()
{
	...
	do
	{
		cout << "Please, enter the maximum length of your snake (note that initially its length is 5):" << endl;
		cin >> length;
	} while (length <= 5);
	s.SetMaxTail(length);
	cout << "Please, enter speed(recommended speed 100):" << endl;
	cin >> speed;
	while (!Gameover)
	{
		s.DrawSnake(headcolor, bodycolor);
		A.DrawMap(color);
		eat.DrawEAT(foodcolor);
		s.crawl();
		GameSpeed(speed);
		s.kb();
		s.control();
