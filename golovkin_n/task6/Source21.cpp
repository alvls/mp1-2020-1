...
private:
	...
	Cell cells[_size][_size];
	Cell cells1[_size][_size];
	Cell cells2[_size][_size];
	int sizeShip;
	int Ship1 = 0;
	int Ship2 = 0;
	int Ship3 = 0;
...
public:
	...
	void ShootPlayer(int _y, int _x)
	{
		...
		if (p == Empty)
			cells1[_y][_x].SetState(Miss);
		else
			cells1[_y][_x].SetState(HitDeck);
	}
	bool GameEnd()
	{
		for (int i = 0; i < _size; i++)
			for (int j = 0; j < _size; j++)
			{
				CellState p = cells[i][j].GetState();
				if (p == Deck)
					return false;
			}
		return true;
	...
	void CreatorShipIi()
	{
		...
		while (count1 < 10)// ѕока кол-во кораблей не 10, он их создает
		{
			if (size4 == 1)
			{
				...
				switch (gorizontal)
				{
				case 0://√оризонтальный 4-х палубник(vertik)
					sizeShip = 4;
					y = rand() % (6 - 0) + 0;
					x = rand() % (9 - 0) + 0;
					for (int i = 0; i < sizeShip; i++)
					...
					break;
				case 1: // ¬ертикальный 4-ч палубник
					sizeShip = 4;
					y = rand() % (9 - 0) + 0;
					x = rand() % (6 - 0) + 0;
					for (int i = 0; i < sizeShip; i++)
				...
				do
				{
					...
					case 1:// вертикальный
						...
						do
						{
							x = rand() % (9 - 0) + 0;
							y = rand() % (8 - 0) + 0;
						} while ((cells1[y][x].GetState() == Deck) && (cells1[y][x].GetState() == No) && (cells1[y][x + 1].GetState() != No));
						if (flag1 == true)
						{
							for (int i = 0; i < sizeShip; i++)
								cells1[y + i][x].SetState(Deck);
